...
public class PayOKHttpWrapper {

    private static final int DEFAULT_TIMEOUT = 125;  //credit pay need more timeout

    private static pay.androidcredit.util.PayOKHttpWrapper sInstance;

    private OkHttpClient okHttpClient;

    private PayOKHttpWrapper() throws Exception {
        initOkHttpClient();
    }

    public static pay.androidcredit.util.PayOKHttpWrapper getInstance() throws Exception {
        if (sInstance == null) {
            sInstance = new pay.androidcredit.util.PayOKHttpWrapper();
        }
        return sInstance;
    }
